...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ Category: "menStyle" });
  let menstyle = {};
  for (let item of products) {
    if (item.title in menstyle) {
      if (
        !menstyle[item.title].Color.includes(item.Color) &&
        item.avaliableQty > 0
      ) {
        menstyle[item.title].Color.push(item.Color);
      }
      if (
        !menstyle[item.title].Size.includes(item.Size) &&
        item.avaliableQty > 0
      ) {
        menstyle[item.title].Size.push(item.Size);
      }
    } else {
      menstyle[item.title] = JSON.parse(JSON.stringify(item));
      if (item.avaliableQty > 0) {
        menstyle[item.title].Color = [item.Color];
        menstyle[item.title].Size = [item.Size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(menstyle)) }, // will be passed to the page component as props
  };
}
